...
- Шрифты: **Tactic Sans** (display, заголовки) + **Montserrat** (текст).
  Tactic Sans подключён: woff2-сабсеты Medium + Bold (~15 КБ/вес) в `public/fonts/`,
  все заголовки рендерятся на Bold (самое тяжёлое доступное начертание);
  для «₽» (нет глифа в шрифте) — фолбэк Montserrat. Детали: `public/fonts/README.md`.
...
## 🔐 Кабинет владельца (CMS)
- Вход: `https://ваш-домен/#admin?key=CYBERX-OMSK-ROOT-2026`, PIN `5500` (сменить можно в кабинете).
- Редактируется **весь контент сайта**: hero (тексты/видео/постер), 3 клуба (контакты, фото, галереи),
  6 зон (тексты, тарифы, фото), прайс (все 3 группы: добавление/удаление тарифов и строк цен),
  главный турнир (включая ISO-дату таймера), акции с промокодами, ссылки (VK/Telegram/email/Langame/App Store/QR).
- Фото можно **загружать прямо из кабинета** (если подключён бэкенд) — файлы хранятся на CMS-сервере.
- **«Черновик»** — сохранение в localStorage браузера (разработка).
- **«Опубликовать»** — публикация на CMS-бэкенд: посетители видят изменения сразу, без перезаливки сайта.

### CMS-бэкенд (`server/`)
Маленький Node-сервер (Express): `GET /api/content`, `PUT /api/content`, `POST /api/upload`.
Деплой на Render/Railway/VPS — 10 минут, инструкция: [`server/README.md`](server/README.md).
Подключение: указать адрес в `src/config.ts` (`CMS_API_BASE`), пересобрать сайт.
Секрет ключа — переменная `CMS_KEY` (сменить в продакшене!).
